...
def create_open_loop_controller(builder, plant, iiwa, X_G, X_O):
    q_trajectory, trajectory_dim = solve_for_iiwa_internal_trajectory_standalone(X_G, X_O)
    ...
    traj_wsg = PiecewisePolynomial.FirstOrderHold([q_trajectory.start_time(), q_trajectory.end_time()],
        np.hstack([[opened], [opened]]))
    wsg_source = builder.AddSystem(TrajectorySource(traj_wsg))

    return demux.get_output_port(0), wsg_source.get_output_port(), q_trajectory.end_time()
